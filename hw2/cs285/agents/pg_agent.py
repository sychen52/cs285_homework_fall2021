...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations,
        ...
        advantages = self.estimate_advantage(observations, rewards_list,
                                             q_values, terminals)
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i] == 1:
                        ...
                        continue
                    delta = rews[i] + values[i + 1] * self.gamma - values[i]
                    advantages[i] = advantages[
                        i + 1] * self.gae_lambda * self.gamma + delta
    ...
    def _discounted_return(self, rewards):
        ...
        sum = 0
        for t_prime, reward in enumerate(rewards):
            sum += (self.gamma**t_prime) * reward
        list_of_discounted_returns = [sum] * len(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        revert = [0]
        for t_prime in range(len(rewards) - 1, -1, -1):
            reward = rewards[t_prime]
            revert.append(reward + revert[-1] * self.gamma)
        list_of_discounted_cumsums = reversed(revert[1:])
